...
dns_hostup_info='HostUp.se
Site: hostup.se
Docs: github.com/acmesh-official/acme.sh/wiki/dnsapi#dns_hostup
Options:
 HOSTUP_API_KEY API Key (read:dns + write:dns scopes)
OptionsAlt:
 HOSTUP_API_BASE API Base URL. Optional.
 HOSTUP_TTL TTL for TXT records. Optional.
 HOSTUP_ZONE_ID Zone ID. Optional.
'
...
_hostup_delete_record_by_id() {
  ...
  if ! _hostup_rest "DELETE" "/dns/zones/$zone_id/records?recordId=$record_id" ""; then
    return 1
  fi
